...
# -------------------------------------------------------------------
# エディタ (vim / nvim) の AI 機能からの一発呼び出しでは整形しない
#
# stop-audit.sh と同じ免除。上の「対象ファイルの決め方」が認めているとおり、
# このフックは作業ツリーの差分すべてを整形対象にする。通常のターンならそれで
# よい (Codex が触ったファイルもそこに含まれる) が、コミットメッセージ生成の
# ような生成専用の呼び出しは *1 ファイルも編集していない*。整形対象は全部
# ユーザの未コミットの変更であり、頼まれてもいない整形が入る。
#
# 実測: `<leader>cx` 相当の呼び出しで、diff を採取したあとに app.ts が
# prettier で書き換えられた (セミコロン付与)。生成されたメッセージが記述して
# いる木と、呼び出し後の木が食い違う。
