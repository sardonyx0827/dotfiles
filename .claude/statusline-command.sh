...
# $'...' で実バイトにしておき、出力は printf '%s' で行う。以前は '\033[0m' 形の
# 文字列を printf "%b" で解釈させていたが、%b は表示データ側のバックスラッシュ
# も解釈する: cwd に `c\components` があると `\c` で出力を打ち切り、以降の
# モデル名・ctx・git・コスト・末尾の RESET まで消えて端末が着色されたまま残る
# (`x\ty` はタブ化)。lint.sh / stop-audit.sh が避けているのと同じ罠。
RESET=$'\033[0m'
