...
- Sonnet: 設計・デバッグ・実装など (デフォルト)
- Opus: 大規模リファクタリング・プロジェクト全体の分析
- 失敗したタスクは上位モデルで再試行すること

## Codex MCP の使い方ガイドライン

- Sonnet/Opus とは異なる視点が必要なタスクに使用すること
- コミット/プッシュ前のコードレビューに積極的に使うこと
- Sonnet/Opus で2回失敗したタスクは Codex MCP に委譲することを検討すること
  (失敗とは、要件を満たす出力が得られないこと、エラーや意図しない出力を含む)
- 関連タスクには可能な限り同じ threadId を使って文脈を維持すること
