...
- 実装速度を重視し、タスクの性質に応じて SubAgents / AgentTeams を積極的に活用すること
- 使用するモデルは、タスクの性質と複雑さに応じてモデル選択ポリシーに従ってモデルを選択すること
- 調整ボトルネックが発生する時は積極的にAgent Teams("teammateMode": "tmux")を使用すること
  特に以下のようなケースで積極的に使用すること :
   - タスクを分割した結果、仕様検討/実装/テスト/レビューと役割分担するのが妥当な場合
   - レビュー・フィードバックループが有効な場合
   - 横断的整合性を維持したい場合
   - 共有タスクを使ったシーケンシャルおよびパラレル実行が有効な場合
   - チームメンバーへの直接介入が必要な場合
