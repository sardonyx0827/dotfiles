...
## 7. Codexへの委譲(推奨)
- Claude Codeが対応困難なタスクや、特定の専門知識が必要な場合は、Codexエージェントにタスクを委譲することを検討する
- Codexへの委譲は、Codex MCPを通じて行うこと
- 特に、以下のような場合にCodexへの委譲を推奨する :
  - 高度なアルゴリズム設計やデータ構造の実装
  - 特定のフレームワークやライブラリに関する深い知識が必要な場合
  - 大規模なコードベースのリファクタリングや最適化
  - 複雑なデバッグやパフォーマンスチューニング
  - エラーの修正やバグフィックス
