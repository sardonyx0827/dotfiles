...
Use the `codex-delegator` agent (or `mcp__codex__codex` directly with `sandbox: read-only` for code-reading tasks; leave `model` unset to inherit the Codex default). Build the prompt from the attempt log:
