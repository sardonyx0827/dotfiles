...
function M:transform_tool(tool)
  ...
  then
    ...
    if not parameters_schema or not parameters_schema.type or not parameters_schema.properties or not parameters_schema.required then
       Utils.error("Gemini Provider: Invalid final parameters_schema for tool '" .. tool.name .. "'", {title="Avante"})
...
function M:parse_messages(opts)
  ...
  if opts.tool_histories and #opts.tool_histories > 0 then
    -- 1. Add the last assistant message that contained the function *call*
    local function_calls = {}
    for _, history in ipairs(opts.tool_histories) do
      if history.tool_use then
        local args = vim.fn.json_decode(history.tool_use.input_json or "{}") -- Gemini expects object
        table.insert(function_calls, { functionCall = { name = history.tool_use.name, args = args } })
      end
    end
    if #function_calls > 0 then add_content(M.role_map.assistant, function_calls) end

    -- 2. Add the function *responses*
    for _, history in ipairs(opts.tool_histories) do
      if history.tool_result then
        local result_content = history.tool_result.content
          or (history.tool_result.is_error and "Error executing tool" or "")
        add_content("function", { -- Gemini uses 'function' role for results
          {
            functionResponse = {
